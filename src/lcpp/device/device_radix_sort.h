...
class DeviceRadixSort : public LuisaModule
{
  ...
  private:
                             ...
                             bool                     is_overwrite_okay)
    {
        ...
        // Zero-initialize all working buffers (required by atomic histogram and decoupled lookback)
        ...
        stream << d_bins_buffer.copy_from(zeros_bins.data())
               << d_ctrs_buffer.copy_from(zeros_ctrs.data()) << synchronize();
        ...
        for(uint current_bit = begin_bit, pass = 0; current_bit < end_bit; current_bit += RADIX_BITS, ++pass)
        {
            ...
            for(uint portion = 0; portion < num_portions; ++portion)
            {
                ...
                stream << d_lookback_buffer.copy_from(zeros_lookback.data()) << synchronize();
